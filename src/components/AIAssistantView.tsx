...
export default function AIAssistantView() {
  ...
  const handleSend = async (textToSend?: string) => {
    ...
    try {
      ...
      const errorMessage: Message = {
        id: String(Date.now() + 1),
        role: 'ai',
        content: `⚠️ Could not complete request: ${err.message || 'Server error'}. Please verify your Gemini API key in Settings if necessary.`,
        timestamp: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })
      };
  ...
  const handleExportChat = () => {
    ...
    a.href = url;
    a.download = `ADO_BISHAL_Assistant_Chat_${new Date().toISOString().split('T')[0]}.txt`;
    a.click();
    ...
    <div className="h-[calc(100vh-140px)] flex flex-col bg-white rounded-2xl shadow-sm border border-slate-300 overflow-hidden">
        ...
        <div className="flex items-center space-x-3.5">
          ...
          <div>
            <div className="flex items-center space-x-2">
              <h2 className="text-lg font-black tracking-tight text-white">ADO BISHAL Assistant</h2>
          ...
          <div key={m.id} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}>
            ...
            <div className={`max-w-[85%] sm:max-w-[75%] space-y-1.5`}>
              ...
              <div className={`text-[10px] text-slate-400 font-mono ${m.role === 'user' ? 'text-right' : 'text-left pl-1'}`}>
                {m.role === 'user' ? 'Staff Admin' : 'ADO BISHAL Assistant'} • {m.timestamp}
              </div>
            </div>
        ...
        {loading && (
          <div className="flex justify-start">
            ...
            <div className="bg-white border border-slate-300 p-4 rounded-2xl rounded-tl-xs shadow-sm flex items-center space-x-3">
              ...
              <span className="text-slate-600 text-sm font-medium">ADO BISHAL Assistant is analyzing cargo database...</span>
            </div>
          </div>
        )}
      ...
      <form onSubmit={(e) => { e.preventDefault(); handleSend(); }} className="p-4 bg-white border-t border-slate-200 shrink-0">
        <div className="flex items-center space-x-3 w-full relative">
          <input
            type="text"
            placeholder="Ask ADO BISHAL Assistant about consignments, markas, transit status, or client drafts..."
            value={input}
